...
class CLASSIFIER:
    def __init__(self,model, _train_X, _train_Y,_test_seen_X,_test_seen_Y,_test_novel_X, _test_novel_Y, seenclasses,novelclasses,
        ...
        np.save("test_seen_X.npy", _test_seen_X.cpu().numpy())
        np.save("test_novel_X.npy", _test_novel_X.cpu().numpy())
    ...
    def val_gzsl(self, test_X, test_label, target_classes):
        ...
        with torch.no_grad():
            ...
            np.save("original_label_zsl.npy", test_label.detach().cpu().numpy())
            np.save("predicted_label_zsl.npy", predicted_label.detach().cpu().numpy())
