...
dataroot = 'D:\model\CRnet相关\CRnet1009\data'
dataset = 'APY'
...
att = np.append(att,np.zeros(64)).reshape(4,64)
...
for episode in range(EPISODE):
    ...
    sample_attributes_mean = sample_attributes.mean(dim = 0)
    distance = 0
    for i in range(sample_attributes.shape[0]):
        distance += torch.norm(sample_attributes[i] - sample_attributes_mean)
    ...
    distance = 0.0000001*distance
    #loss = mse(relations,one_hot_labels)-distance+juli
    loss = mse(relations,one_hot_labels)-distance
    ...
    if (episode+1)%100 == 0:
            print("episode:",episode+1,"loss1 %.4f,distance %.4f" % (loss.item(),distance.item()))
